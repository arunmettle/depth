...
$reportPath = Join-Path $ReportDirectory "goal-7-validation-$timestamp.json"

if (-not $env:VALIDATION_REPORT_PATH) {
  $env:VALIDATION_REPORT_PATH = $reportPath
}

Write-Output "Goal 7 validation report will be written to $($env:VALIDATION_REPORT_PATH)"
...
Write-Output ""
Write-Output "Goal 7 validation completed successfully."
