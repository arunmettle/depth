...
$reportPath = Join-Path $ReportDirectory "goal-5-validation-$timestamp.json"

if (-not $env:VALIDATION_REPORT_PATH) {
  $env:VALIDATION_REPORT_PATH = $reportPath
}

Write-Output "Goal 5 validation report will be written to $($env:VALIDATION_REPORT_PATH)"
...
Write-Output ""
Write-Output "Goal 5 validation completed successfully."
