...
$reportPath = Join-Path $ReportDirectory "goal-10-validation-$timestamp.json"

if (-not $env:VALIDATION_REPORT_PATH) {
  $env:VALIDATION_REPORT_PATH = $reportPath
}

Write-Output "Goal 10 validation report will be written to $($env:VALIDATION_REPORT_PATH)"
