...
$reportPath = Join-Path $ReportDirectory "goal-4-validation-$timestamp.json"

if ($ExerciseCrud.IsPresent) {
  $env:VALIDATION_EXERCISE_CRUD = "true"
}
...
Write-Output "Goal 4 validation report will be written to $($env:VALIDATION_REPORT_PATH)"
...
Write-Output ""
Write-Output "Goal 4 validation completed successfully."
