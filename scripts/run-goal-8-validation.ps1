...
$reportPath = Join-Path $ReportDirectory "goal-8-validation-$timestamp.json"

if ($TriggerAlert.IsPresent) {
  $env:VALIDATION_TRIGGER_ALERT = "true"
}
...
Write-Output "Goal 8 validation report will be written to $($env:VALIDATION_REPORT_PATH)"
...
Write-Output ""
Write-Output "Goal 8 validation completed successfully."
